...
nearest_event(Ev, Name,Ar,Idx,[Head|Tail], Thappens,Tcall):-
			var(Head), println('RETRIeVING POINT'(Name)),
			indexer<-retrieve_point(Name,Ar,Idx,Tcall) returns Point, println('Retrieving Point'(Point)),  \+ var(Point),
...
member_iterator(M, Iterator):-
	Iterator<-hasNext, 
	Iterator<-next returns M, println(M).
